...
struct CreateProfileView: View {
  ...
  private func genderCard(for gender: Gender) -> some View {
    ...
    Button {
      ...
      .shadow(
        color: isSelected ? Color.accentColor.opacity(0.25) : Color.clear, radius: 10, x: 0, y: 4)
    }
    .buttonStyle(.plain)
    ...
    ScrollView {
      VStack(spacing: 38) {
        VStack(spacing: 3) {
          ...
          Text(
            "We'll use this data to calculate your calorie needs and suggest optimal workout volumes."
          )
          .font(.system(size: 14))
          .frame(maxWidth: .infinity, alignment: .leading)
        }
